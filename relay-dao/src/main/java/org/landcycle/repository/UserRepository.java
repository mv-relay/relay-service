...
public interface UserRepository extends Repository<UserEntity, String> 
{
	...
	@Query(nativeQuery = true, value = "select u.*,f.*, ( 6371 * acos( cos( radians(:latitudine) ) * cos( radians( lat ) ) * cos( radians( lng ) - radians(:longitudine) ) + sin( radians(:latitudine) ) * sin( radians( lat ) ) ) ) AS distance from User u JOIN Forsale AS f on u.mail = f.mailvend  where f.mailvend <> :mailvend  HAVING distance < 50 ORDER BY distance LIMIT 0 , 50")
