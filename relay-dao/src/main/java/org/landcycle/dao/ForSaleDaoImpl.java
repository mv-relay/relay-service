...
public class ForSaleDaoImpl extends HibernateDaoSupport implements ForSaleDao {
	...
	protected static final Logger log = Logger.getLogger(ForSaleDaoImpl.class.getName());
	/*
	 * (non-Javadoc)
	 * 
	...
	public List<ForSaleEntity> findByQuery(ForSaleEntity user) {
		...
		if (user.getLat() != null && user.getLng() != null) {
			...
			query.setParameter("mailvend", "valerio.artusi@gmail.com");
		}else{
...
//					.createSQLQuery("select f.id, f.name,f.img,f.description,f.tags,f.mailvend,f.mailacq,f.optional,f.citta, f.lat, f.lng, u.mail,u.nome,u.avatar  FROM forsale f , user u where u.mail = f.mailvend ");
		...
		log.debug("Return Dao : " + ret);
		return ret;
	}
	
	@Override
	public List<UserEntity> findByQueryz(ForSaleEntity user) {
		Session session = getSession(false);
		Query query = null;
		if (user.getLat() != null && user.getLng() != null) {
			//cerca nel raggio di 50 km
			query = session.createSQLQuery("select distinct {u.*} , ( 6371 * acos( cos( radians(:latitudine) ) * cos( radians( lat ) ) * cos( radians( lng ) - radians(:longitudine) ) + sin( radians(:latitudine) ) * sin( radians( lat ) ) ) ) AS distance from User u , Forsale f where f.mailvend = u.mail and f.mailvend <> :mailvend HAVING distance < 1150 ORDER BY distance LIMIT 0 , 50")// where u.mail = f.mailvend and f.mailvend <> :mailvend  HAVING distance < 50 ORDER BY distance LIMIT 0 , 50")  
...
//					.createSQLQuery("select f.id, f.name,f.img,f.description,f.tags,f.mailvend,f.mailacq,f.optional,f.citta, f.lat, f.lng, u.mail,u.nome,u.avatar  FROM forsale f , user u where u.mail = f.mailvend ");
		...
		List<UserEntity> ret = query.list();
